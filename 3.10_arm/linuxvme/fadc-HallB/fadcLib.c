...
faStatus(int id, int sflag)
{
  ...
  unsigned int val, trig_mode[16], inverted[16];
  ...
  printf("  Ch| Readout - TET | Trigger - TET | GAIN   | PED     | DELAY  | TRGMODE| INVERT\n");
  printf("  --|---------------|---------------|--------|---------|--------|--------|-------\n");
  for(ii=0;ii<FA_MAX_ADC_CHANNELS;ii++)
  {
    if(trig_mode[ii]) trig_mode_string = "DISC";
    else              trig_mode_string = "PULSE";
    printf("  %2d|          %4d |          %4d |%7.3f |%8.3f |%3d | %s | %5d\n", ii, tet_readout[ii], tet_trg[ii], gain_trg[ii], ped_trg[ii], delay[ii], trig_mode_string, inverted[ii]);
  }
