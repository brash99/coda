...
faStatus(int id, int sflag)
{
  ...
  NSB =  nsb_reg&FA_ADC_NSB_READBACK_MASK;
  ...
  adc_option  = (adcConf[0]&FA_ADC_PROC_MASK)? 10 : 9;
  ...
  printf("   Lookback          (PL) = %4d ns   Time Window     (PTW) = %4d ns\n",
	 PL, PTW);
  printf("   Time Before Peak (NSB) = %4d ns   Time After Peak (NSA) = %4d ns\n",
	 NSB, NSA);
  printf("   Max Peak Count  (MNoP) = %4d \n",NP);
  printf("   Samples in Pulse Pedestal Sum            (NPED) = %4d\n",
	 NPED + 1);
  printf("   Max Pedestal for readback pedestal sum (MAXPED) = %4d\n",
	 MAXPED);
  printf("   Number of Samples over TET required      (NSAT) = %4d\n",
	 NSAT);
  ...
  if(ctrl1 & (FA_CTRL1_SUPPRESS_TT | FA_CTRL1_SUPPRESS_TT2))
    {
      if(FA_CTRL1_SUPPRESS_TT2)
	printf("  MSB of Trigger Timestamp --- SUPPRESSED\n");
      else
	printf("  All Trigger Timestamps --- SUPPRESSED\n");
    }
  ...
  for(ifa=0; ifa<nfadc; ifa++)
    {
      ...
      printf("%2d   ",(st[id].adc_config[0] & FA_ADC_PROC_MASK)? 10 : 9);
  ...
  for(ifa=0; ifa<nfadc; ifa++)
    {
      ...
      printf("%d                ", (((st[id].adc_config[0] &FA_ADC_CONFIG1_TNSAT_MASK)>>12) + 1)*FA_ADC_NS_PER_CLK);
...
faSetNormalMode(int id, int opt)
{
  ...
  if((id<=0) || (id>21) || (FAp[id] == NULL)) {
    logMsg("faSetNormalMode: ERROR : FADC in slot %d is not initialized \n",id,0,0,0,0,0);
    return;
  }
...
/**
 *  @ingroup Config
 *  @brief Suppress all, or just the MSB, of the trigger time words from the datastream.
 *  @param id Slot number
 *  @param suppress Suppression Parameter
 * <pre>
 ...
 */
int
faSuppressTriggerTime(int id, int suppress)
{
  int bits=0;
  if(id==0) id=fadcID[0];
  ...
  if((suppress < 0) || (suppress > 2))
    {
      printf("%s: ERROR: Invalid suppress parameter (%d)\n",
  ...
  switch(suppress)
    ...
    case 1:
      bits = FA_CTRL1_SUPPRESS_TT2;
      break;

    case 2:
      bits = FA_CTRL1_SUPPRESS_TT;
      break;
    }

  FALOCK;
  vmeWrite32(&FAp[id]->ctrl1,
	     (vmeRead32(&FAp[id]->ctrl1) & ~(FA_CTRL1_SUPPRESS_TT | FA_CTRL1_SUPPRESS_TT2)) |
	     bits);
  FAUNLOCK;
 ...
 */
int
faGSuppressTriggerTime(int suppress)
{
  int ii, res, rval = OK;
  ...
  for (ii=0;ii<nfadc;ii++)
    {
      res = faSuppressTriggerTime(faSlot(ii), suppress);
      if(res<0)
	{
	  printf("ERROR: slot %d, in faSuppressTriggerTime(...)\n", faSlot(ii));
	  rval = ERROR;
	}
    }
...
{
  unsigned int reg=0;
#ifdef USE_PROC_TRIG_RCV_DONE
  unsigned int stat=0;
  int itimeout=0, timeout=10;
#endif /* USE_PROC_TRIG_RCV_DONE */
...
/**
 *  @ingroup Config
 *  @brief Check and make sure readout threshold for each channel is non zero
 *  @param id Slot number
 *  @return OK if successful, otherwise ERROR.
 */
int
faCheckThreshold(int id)
{
...
faSetThreshold(int id, unsigned short tvalue, unsigned short chmask)
{
  ...
  if(tvalue == 0)
    {
      logMsg("faSetThreshold: ERROR: Invalid threshold (%d).\n",
	     tvalue, 0, 0, 0, 0, 0);
      return ERROR;
    }
...
faReadAllChannelSamples(int id, volatile unsigned int *data)
{
  ...
  for(ichan=0; ichan<FA_MAX_ADC_CHANNELS; ichan++)
    {
      read = vmeRead32(&FAp[id]->adc_status[2]);

      if(read & (1<<14))
	{
	  read = (read & FA_ADC_STATUS2_CHAN_DATA_MASK) | (1<<15);
	}
      else
	read &= FA_ADC_STATUS2_CHAN_DATA_MASK;
...
faDisableScalers(int id)
{
  ...
  FALOCK;
  vmeWrite32(&FAp[id]->scaler_ctrl, 0);
  FAUNLOCK;
...
faGetA32(int id)
{
  ...
  if(FApd[id])
    {
      rval = (unsigned long)FApd[id] - fadcA32Offset;
    }
  else
    {
...
faGetA32M()
{
  ...
  if(FApmb)
    {
      rval = (unsigned long)FApmb - fadcA32Offset;
    }
  else
    {
 ...
 */
void
faGDataInsertAdcParameters(int enable)
{
  int ifadc;

  for(ifadc=0;ifadc<nfadc;ifadc++)
    faDataInsertAdcParameters(faSlot(ifadc), enable);
 ...
 */
void
faGDataSuppressTriggerTime(int suppress)
{
  int ifadc;

  for(ifadc=0;ifadc<nfadc;ifadc++)
    faDataSuppressTriggerTime(faSlot(ifadc), suppress);
...
faSetDataFormat(int id, int format)
{
  ...
  FALOCK;
  vmeWrite32(&FAp[id]->ctrl1,
	     (vmeRead32(&FAp[id]->ctrl1) & ~FA_CTRL1_DATAFORMAT_MASK)
	     | format);
  FAUNLOCK;
...
int
faStateArmBuffer(int id, int enable)
{
  if(id==0) id=fadcID[0];

  if((id<=0) || (id>21) || (FAp[id] == NULL))
    {
      logMsg("faStateArmBuffer: ERROR : ADC in slot %d is not initialized \n",
	     id,2,3,4,5,6);
      return ERROR;
    }
  ...
  FALOCK;
  if(enable)
    vmeWrite32(&FAp[id]->state.csr, FA_STATE_CSR_ARM_BUFFER);
  else
    vmeWrite32(&FAp[id]->state.csr, 0);
  FAUNLOCK;
 ...
 */
int
faStateReadBuffer(int id, volatile unsigned int *data, int nwords)
{
  ...
  if((id<=0) || (id>21) || (FAp[id] == NULL))
    {
      logMsg("faStateReadBuffer: ERROR : ADC in slot %d is not initialized \n",
	     id,2,3,4,5,6);
      return ERROR;
    }
  ...
  ndata = vmeRead32(&FAp[id]->state.csr) & FA_STATE_CSR_BUFFER_WORDS_MASK;

  if(ndata == 0)
    {
      logMsg("faStateReadBuffer(%d): WARN: No words in State Machine buffer\n",
	     id, 2, 3, 4, 5, 6);
      rval = 0;
    }
  else
    {
      if(ndata > nwords)
	{
	  logMsg("faStateReadBuffer(%d): WARN: %d words remain in State Machine buffer\n",
		 id, ndata, 3, 4, 5, 6);

	}
      for(idata = 0; idata < ndata; idata++)
	{
	  data[idata] = vmeRead32(&FAp[id]->state.value) & FA_STATE_VALUE_MASK;
	}
      rval = ndata;
    }
 ...
 */
int
faStateMap(unsigned int state_value)
{
  int rval=0;
...
faStatePrintBuffer(int id)
{
  ...
  if((id<=0) || (id>21) || (FAp[id] == NULL))
    {
      logMsg("faStatePrintBuffer: ERROR : ADC in slot %d is not initialized \n",
	     id,2,3,4,5,6);
      return ERROR;
    }

  nwords = faStateReadBuffer(id, (volatile unsigned int *)&data, 0xff);
  if(nwords < 0)
    {
      logMsg("faStatePrintBuffer(%d): ERROR: Unable to retreive state machine data\n", id, 2, 3, 4, 5, 6);
      return ERROR;
    }
...
unsigned int faDebugRegs[FA_DEBUG_OUTPUT_NREGS] =
  {
    0x04, 0x08, 0x0c, 0x10, 0x30,
    0x34, 0x38, 0x48, 0x4c, 0x70,

    0x74, 0x78, 0x7c, 0x80, 0x84,
    0xa0, 0xa4, 0xa8, 0xac, 0xb0,

    0xb4, 0xb8, 0xbc, 0xc0, 0xd0,
    0xd4, 0xd8, 0xdc, 0xe0, 0xe4,

    0xe8, 0xec, 0xf0, 0xfc, 0x300,
    0x304, 0x308, 0x30C, 0x310, 0x314,

    0x318, 0x31C, 0x320, 0x324, 0x328,
    0x32c, 0x330, 0x334, 0x338, 0x33c,

    0x400, 0x408, 0x40c, 0x410, 0x500,
    0x504, 0x508
  };

int
faDebugOutput(int id, volatile unsigned int *data, int nwords)
{
  int rval = 0, ireg = 0, idata = 0;
  unsigned int *faP = NULL;

  if(id==0) id=fadcID[0];

  if((id<=0) || (id>21) || (FAp[id] == NULL))
    {
      logMsg("faDebugOutput: ERROR : FADC in slot %d is not initialized \n",id,0,0,0,0,0);
      return(ERROR);
    }

  if(data==NULL)
    {
      logMsg("faDebugOutput: ERROR: Invalid Destination address\n",0,0,0,0,0,0);
      return(ERROR);
    }

  /* Reduce number of regs readout, if nwords is smaller than nregs */
  nwords = (nwords < FA_DEBUG_OUTPUT_NREGS) ? nwords : FA_DEBUG_OUTPUT_NREGS;

  faP = (unsigned int *)FAp[id];

  FALOCK;
  for(ireg = 0; ireg < nwords; ireg++)
    {
      data[idata++] = vmeRead32(&faP[(faDebugRegs[ireg] >> 2)]);
    }
  rval = idata;
  FAUNLOCK;
...
int
faGetDebugRegs(int id, volatile unsigned int *data, int nwords)
{
  int rval = 0, ireg = 0, idata = 0;
  ...
  if((id<=0) || (id>21) || (FAp[id] == NULL))
    {
      logMsg("faDebugRegs: ERROR : FADC in slot %d is not initialized \n",id,0,0,0,0,0);
      return(ERROR);
    }

  if(data==NULL)
    {
      logMsg("faDebugRegs: ERROR: Invalid Destination address\n",0,0,0,0,0,0);
      return(ERROR);
    }
...
faPrintDebugOutput()
{
  int rval = 0, ifa = 0, id = 0, ireg = 0, irow = 0, nrows = 0;
  volatile unsigned int data[FA_MAX_BOARDS+1][FA_DEBUG_OUTPUT_NREGS * 2];
  const int nRegsPerRow = 6;

  if(id==0) id=fadcID[0];

  if((id<=0) || (id>21) || (FAp[id] == NULL))
    {
      logMsg("faPrintDebugOutput: ERROR : FADC in slot %d is not initialized \n",id,0,0,0,0,0);
      return(ERROR);
    }
  ...
  for(ifa = 0; ifa < nfadc; ifa++)
    {
      id = faSlot(ifa);
      rval = faDebugOutput(id, (volatile unsigned int *)&data[id], FA_DEBUG_OUTPUT_NREGS);
      if (rval < 0)
	memset((char *)data[id], -1, sizeof(data[id]));
    }

  printf("\n\n");
  printf("FADC Debug Register Output\n");
  printf("--------------------------------------------------------------------------------\n");

  /* Number of rows = nregs / (regs per row) */
  nrows = (rval / nRegsPerRow) + ((rval % nRegsPerRow) ? 1 : 0);

  for(irow = 0; irow < nrows; irow++)
    {
      printf("Slot   ");
      for(ireg = (irow * nRegsPerRow); ireg < (nRegsPerRow + (irow * nRegsPerRow)); ireg++)
	{
	  printf("  0x%04x",
		 faDebugRegs[ireg]);
	  if( ((ireg + 1) % nRegsPerRow) && (ireg < (FA_DEBUG_OUTPUT_NREGS - 1)))
	    printf("     ");
	  else
	    {
	      printf("\n");
