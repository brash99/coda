...
/**
 * @ingroup Config
 * @brief Enable/Disable the pulse rising or trailing edge in readout.
 * @param id Slot Number
 * @param rise Rising Edge Enable Flag
 * @param fall Falling Edge Enable Flag
 * @return OK if successful, otherwise ERROR
 */

int
vfTDCSetEdgeReadout(int id, int rise, int fall)
{
  if(id==0) id=vfTDCID[0];
  ...
  /* Flip the logic for the register bits */
  rise = (rise) ? 0 : 1;
  fall = (fall) ? 0 : 1;

  VLOCK;
  vmeWrite32(&TDCp[id]->pl,
	     (vmeRead32(&TDCp[id]->pl) & ~VFTDC_PL_EDGE_DISABLE_MASK) |
	     (rise << 14) | (fall << 15));
  VUNLOCK;
...
/**
 * @ingroup Config
 * @brief Get the status of the pulse rising or trailing edge in readout.
 * @param id Slot Number
 * @param *rise Where to return Rising Edge Enable Flag
 * @param *fall Where to return Falling Edge Enable Flag
 * @return OK if successful, otherwise ERROR
 */

int
vfTDCGetEdgeReadout(int id, int *rise, int *fall)
{
  unsigned int reg = 0;
  if(id==0) id=vfTDCID[0];
  ...
  VLOCK;
  reg = vmeRead32(&TDCp[id]->pl) & ~VFTDC_PL_EDGE_DISABLE_MASK;

  *rise = (reg & VFTDC_PL_RISING_EDGE_DISABLE)>>14;
  *fall = (reg & VFTDC_PL_RISING_EDGE_DISABLE)>>15;
  VUNLOCK;

  /* Flip the logic of the register bits */
  *rise = (*rise) ? 0 : 1;
  *fall = (*fall) ? 0 : 1;
