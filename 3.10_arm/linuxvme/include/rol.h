...
/* Include the memory partition structures and routines (part of CODA_ROC distribution) */
#include "libpart.h"

/* The following structure is used per readout list to store it's global data  (part of CODA_ROC distribution) */
#include "rolInt.h"
...
/* Create Macros for setting and clearing ROL Error conditions */
#define ROL_SET_ERROR    rol->nounload = 1;
#define ROL_CLEAR_ERROR  rol->nounload = 0;
...
void INIT_NAME(rolp, cmd)
     rolParam rolp;
     int cmd;
{
      rolp->daproc = cmd;
      if ((cmd != DA_DONE_PROC)&&(cmd != DA_POLL_PROC))
	printf("ROL: rolp->daproc = %d\n",cmd);
	...
	break;
      case DA_POLL_PROC:
	printf("ROL: WARN: __poll() no longer supported\n");
	break;
      case DA_DONE_PROC:
	printf("ROL: WARN: __done() no longer supported\n");
	break;
      /*case DA_RESET_PROC:
	__reset();
	break;*/
      default:
	printf("ROL: WARN: unsupported rol action = %d\n",cmd);
	break;
      }

      if ((cmd != DA_DONE_PROC)&&(cmd != DA_POLL_PROC))
      printf("ROL: Exiting (%d)\n",rolp->daproc);
}
