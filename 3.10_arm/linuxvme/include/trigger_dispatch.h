...
{
  unsigned int theType,theSource;
  int go_on;
  DANODE *theNode;
  ...
  while ((rol->dispQ->list.c) && (go_on)) {
	...
	(*wrapperGenerator)(theType);
      ...
      rol->dabufpi = (unsigned int *) &input_event__->length;
#else
      (*(rol->nevents))++;
#endif
...
static int theIntHandler(int theSource)
{
  if (theSource == 0) return(0);
  {
    ...
    theNode->reader = (void *) trigRtns[theSource];
    listAdd(&rol->dispQ->list, theNode);
    if (!dispatch_busy)
      cdodispatch();
    UNLOCKINTS;
...
static int cdopolldispatch()
{
  ...
  if (!poolEmpty) {
	...
	if ( (theNode = (DANODE *)(*syncTRtns[theSource])(Tcode[theSource]))) {
	  stat = 1;
	  {
	    LOCKINTS;
	    if (theNode)
