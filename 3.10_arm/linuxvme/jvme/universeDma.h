...
void sysVmeDmaReset(void);
void sysVmeReset(void);
unsigned int sysUnivSetUserAM(int amcode1, int amcode2);
unsigned int sysUnivSetLSI(unsigned short id, unsigned short vas);
...
int sysUnivDmaConfig(unsigned int addrType, unsigned int dataType);
...
#ifndef EXTENDED_VME
#define CPU_PCI_MEM_ADRS    0xfd050000   /* Special French Kernel for Hall A Compton Polarimeter*/
...
#else
#define CPU_PCI_MEM_ADRS    0xfd050000   /* Special French Kernel for Hall A Compton Polarimeter*/
/*#define CPU_PCI_MEM_ADRS    0x83000000 */
...
#else
extern unsigned int univBaseAdrs;
#define UNIVERSE_BASE_ADRS  univBaseAdrs
#endif
