...
# user-friendly remote procdure calls (RPC's) on top of the 
...
proc dp_Host {str} {
	...
	default {global dp_Acl; return $dp_Acl}
    }
    return $rv
}
...
proc dp_AcceptRPCConnection {server loginFunc checkCmd status file} {
    ...
    dp_filehandler $newFile re "dp_ProcessRPCCommand $server"
...
proc dp_MakeRPCClient {host port {checkCmd none}} {
    ...
    dp_filehandler $client r "dp_ProcessRPCCommand 0"
...
proc dp_MakeRPCServer  {{port 0} {loginFunc dp_CheckHost} {checkCmd none}
    ...
    dp_filehandler $server re "dp_AcceptRPCConnection $server $loginFunc $checkCmd"
...
#
proc tkerror {info} {
    global tk_library errorInfo
    if [info exists tk_library] {
	tkerror.tk $info
    } else {
	puts "Background ERROR in Tcl code..........."
	puts "$info \n $errorInfo"
	puts ""
    }
}
