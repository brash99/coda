...
proc tixComboBox::SetBindings {w} {
	...
	bind $data(w:arrow) <1>               "tixComboBox::ArrowDown $w"
	bind $data(w:arrow) <ButtonRelease-1> "tixComboBox::ArrowUp   $w"
...
tixBind TixComboBox <FocusOut>  {
	...
	if [info exists %W(cancelTab)] {
	    ...
	    if {[set %W(-state)] != "disabled"} {
		if {[set %W(-selection)] != [set %W(-value)]} {
...
tixBind TixComboEntry <Return>	{	
    tixComboBox::EntReturnKey [tixGetMegaWidget %W]
}
...
tixBind TixComboEntry <Tab> 	{
	...
	if [tixComboBox::EntTab [tixGetMegaWidget %W]] {
	    break
	}
    }
}
...
tixBind TixComboWid <ButtonRelease-1> {
    tixComboBox::WidUp [tixGetMegaWidget %W]
}
...
proc tixComboBox::Cancel {w} {
    ...
    if {[tixComboBox::LbGetSelection $w] != $data(-selection)} {
	tixComboBox::ClearListboxSelection $w
    }
}
...
proc tixComboBox::config-value {w value} {
    ...
    if {[tixComboBox::LbGetSelection $w] != $value} {
	tixComboBox::ClearListboxSelection $w
    }
}
